...
import acgan_pytorch.models as models
from acgan_pytorch.models import discriminator
from acgan_pytorch.utils import init_torch_seeds
from acgan_pytorch.utils import select_device
from acgan_pytorch.utils import weights_init
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.auxiliary_criterion = nn.CrossEntropyLoss().to(self.device)
        logger.info(f"Loss function:\n"
                    f"\tAdversarial loss is BCELoss\n"
                    f"\tAuxiliary loss is CrossEntropyLoss")
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (image, target) in progress_bar:
                real_images = image.to(self.device)
                real_target = target.to(self.device)
                batch_size = real_images.size(0)

                # A set of noises and targets is generated randomly.
                noise = torch.randn(batch_size, 100, 1, 1, device=self.device)
                fake_target = torch.randint(0, args.num_classes, (batch_size,), device=self.device)

                real_label = torch.full((batch_size,), 1, dtype=real_images.dtype, device=self.device)
                fake_label = torch.full((batch_size,), 0, dtype=real_images.dtype, device=self.device)
                ...
                # train with real
                real_pred, real_aux = self.discriminator(real_images)
                errD_real_adv = self.adversarial_criterion(real_pred, real_label)
                errD_real_aux = self.auxiliary_criterion(real_aux, real_target)
                errD_real = errD_real_adv + errD_real_aux
                errD_real.backward()
                D_x = real_pred.mean().item()

                # Generated fake image
                fake_image = self.generator(noise, fake_target)

                # train with fake
                fake_pred, fake_aux = self.discriminator(fake_image.detach())
                errD_fake_adv = self.adversarial_criterion(fake_pred, fake_label)
                errD_fake_aux = self.auxiliary_criterion(fake_aux, fake_target)
                errD_fake = errD_fake_adv + errD_fake_aux
                errD_fake.backward()
                errD = errD_real + errD_fake
                D_G_z1 = fake_pred.mean().item()

                # Update discriminator optimizer gradient.
                ...
                fake_pred, fake_aux = self.discriminator(fake_image)
                errG_adv = self.adversarial_criterion(fake_pred, real_label)
                errG_aux = self.auxiliary_criterion(fake_aux, real_target)
                errG = errG_adv + errG_aux
                errG.backward()
                D_G_z2 = fake_pred.mean().item()

                # Update generator optimizer gradient.
                self.optimizer_g.step()

                # Calculate discriminator accuracy
                pred = np.concatenate([real_aux.data.cpu().numpy(), fake_aux.data.cpu().numpy()], axis=0)
                target = np.concatenate([real_label.data.cpu().numpy(), fake_label.data.cpu().numpy()], axis=0)
                acc = np.mean(np.argmax(pred, axis=1) == target)
